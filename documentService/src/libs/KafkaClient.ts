...
class KafkaClient {
  static selfInstance: KafkaClient;

  private kafka: Kafka;
  private consumer: Consumer;
  private producer: Producer;
  private admin: Admin;
  private constructor({ brokers, clientId }: IKafkaClient) {
  ...
  static getInstance = () => {
    ...
    if (params) {
      if (!KafkaClient.selfInstance) {
        const instance = new KafkaClient(params);
        KafkaClient.selfInstance = instance;
      }
    }

    return KafkaClient.selfInstance;
  };
